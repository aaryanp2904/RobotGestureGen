#!/usr/bin/env python3
"""Generate latent-diffusion predictions for multiple BEAT2 clips."""
...
try:
    from .infer_nao import generate
except ImportError:
    from infer_nao import generate
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate latent-diffusion predictions for multiple BEAT2 clips")
    ...
    print(f"[BATCH-LD] Generating {len(clip_ids)} clips into {output_dir}")
    failures = []
    for idx, clip_id in enumerate(clip_ids, start=1):
        print("\n" + "=" * 70)
        print(f"[BATCH-LD] {idx}/{len(clip_ids)} {clip_id}")
        print("=" * 70)
        try:
    ...
    if failures:
        ...
        print(f"[BATCH-LD] Completed with {len(failures)} failures: {failure_path}")
    else:
        print("[BATCH-LD] Completed without failures")
