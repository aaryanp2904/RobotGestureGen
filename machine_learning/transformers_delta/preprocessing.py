#!/usr/bin/env python3
"""Preprocess BEAT2 for the delta-transformer baseline without energy outputs.

This variant uses the same conditioning as the standard Transformer baseline,
but trains the model to predict per-frame NAO joint deltas instead of absolute
joint angles:
...
DEFAULT_OUTPUT_DIR = "/vol/bitbucket/ap1922/BEAT2_NAO_Transformer_Delta_Preprocessed"
...
def write_transformer_lmdb(
    ...
    for split, rows in sorted(rows_by_split.items()):
        ...
        metadata = {
            ...
            "target_representation": "nao_joint_deltas",
            "target_type": "nao_joint_deltas",
            ...
            "nao_delta_mean": stats["nao_delta_mean"],
            "nao_delta_std": stats["nao_delta_std"],
            "motion_vel_mean": stats["motion_vel_mean"],
            "motion_vel_std": stats["motion_vel_std"],
            "motion_delta_mean": stats["motion_delta_mean"],
            "motion_delta_std": stats["motion_delta_std"],
        }
...
def preprocess(args: argparse.Namespace) -> None:
    ...
    stats = {
        ...
        "nao_delta_mean": vel_mean.tolist(),
        "nao_delta_std": vel_std.tolist(),
        "motion_vel_mean": vel_mean.tolist(),
        "motion_vel_std": vel_std.tolist(),
        "motion_delta_mean": vel_mean.tolist(),
        "motion_delta_std": vel_std.tolist(),
        "speaker_id_map": speaker_id_map,
        "speaker_dim": len(speaker_id_map),
    ...
    print("\n" + "=" * 70)
    print("  BEAT2 DELTA TRANSFORMER PREPROCESSING COMPLETE")
    print("=" * 70)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Preprocess BEAT2 for the delta-transformer baseline without energy outputs"
    )
    ...
    parser.add_argument("--target-mode", choices=["delta"], default="delta")
    ...
    if args.target_mode != "delta":
        raise ValueError("transformers_delta preprocessing only supports --target-mode delta")
