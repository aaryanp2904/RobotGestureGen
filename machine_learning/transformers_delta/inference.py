#!/usr/bin/env python3
"""Run BEAT2/NAO inference with a delta GestureTransformer checkpoint."""
...
from beat2_nao.config import AUDIO_DIR, AUDIO_SR, MOCAP_FPS, TEXTGRID_DIR  # noqa: E402
from beat2_nao.nao_constants import NAO_JOINTS, NAO_LIMITS, NAO_MAX_VEL  # noqa: E402
...
def resolve_target_mode(model_config: dict, dataset_metadata: dict, stats: dict) -> str:
    ...
    return modes[0] if modes else "delta"
...
def validate_contract(model_config: dict, dataset_metadata: dict, stats: dict) -> str:
    ...
    if target_mode != "delta":
        raise ValueError(
            f"transformers_delta inference expects a delta checkpoint, got target_mode={target_mode!r}"
        )
...
def reconstruct_nao_angles(pred_norm: np.ndarray, stats: dict, target_mode: str) -> np.ndarray:
    if target_mode != "delta":
        raise ValueError(f"transformers_delta inference only supports target_mode='delta', got {target_mode!r}")
...
def generate(args: argparse.Namespace) -> np.ndarray:
    ...
    save_metadata(
        output_path,
        {
            ...
            "reconstruction": "nao_mean_plus_cumulative_predicted_deltas",
